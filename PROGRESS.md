...
### Deploy — automatic on every push to `main` (working, confirmed 2026-08-11)
`.github/workflows/refresh.yml` runs on `push` to `main`, nightly at 11:00 UTC, and on demand. One job
re-runs the snapshot against live Sleeper data, builds, and publishes via GitHub's official Pages deploy
(`configure-pages` + `upload-pages-artifact` + `deploy-pages`).

**So a commit is the deploy.** `web/public/data.json` is gitignored but regenerated by that job, which
means data-shaped changes ship on merge just like UI changes — there is nothing to run by hand and no
need to caveat this when landing a fix. (An earlier note here claimed the nightly deploy was broken and
needed a one-time Pages source change; that was resolved and the note was stale.)
